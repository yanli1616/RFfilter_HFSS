...
def create_region(oEditor,offset,
    ...
    RegionParameters=["NAME:RegionParameters",
    ...
    return oEditor.CreateRegion(RegionParameters, Attributes)
...
def sweep_along_path(oEditor, obj_name_list, path):
    '''
    Sweeps the specified 1D or 2D parts along the specified path
    Path is the last 1D object in selections
    ...
    path : list
        List of the specified path
    '''
    selections=",".join(obj_name_list+path) ## very important: no space after comma, otherwise HFSS cannot recognize the path

    oEditor.SweepAlongPath(["NAME:Selections",
...
def set_units(oEditor, units, rescale=True):
    oEditor.SetModelUnits(["NAME:Units Parameter", "Units:=", units, "Rescale:=", rescale])